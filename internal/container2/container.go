...
func MetricsWithARGS(args *common.ARGS, promProtocol, promPort string, debug bool, currentTime time.Time) string {
	//Setup variables used in the code.
	var errors = ""
	var historyInterval time.Duration
	historyInterval = 0
	...
	start, end = prometheus.TimeRange(*args.Interval, *args.IntervalSize, currentTime, historyInterval)
	range5Min := v1.Range{Start: start, End: end, Step: time.Minute * 5}

	promaddress = promProtocol + "://" + *args.PromAddress + ":" + promPort
	...
	//Container workloads
	query = queryPrefix + `round(sum(irate(container_cpu_usage_seconds_total{name!~"k8s_POD_.*"}[5m])) by (instance,pod` + labelSuffix + `,namespace,container` + labelSuffix + `)*1000,1)` + querySuffix
	errors += getWorkload(promaddress, "cpu_mCores_workload", "CPU Utilization in mCores", query, "max", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)
	errors += getWorkload(promaddress, "cpu_mCores_workload", "Prometheus CPU Utilization in mCores", query, "avg", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)

	query = queryPrefix + `sum(container_memory_usage_bytes{name!~"k8s_POD_.*"}) by (instance,pod` + labelSuffix + `,namespace,container` + labelSuffix + `)` + querySuffix
	errors += getWorkload(promaddress, "mem_workload", "Raw Mem Utilization", query, "max", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)
	errors += getWorkload(promaddress, "mem_workload", "Prometheus Raw Mem Utilization", query, "avg", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)

	query = queryPrefix + `sum(container_memory_rss{name!~"k8s_POD_.*"}) by (instance,pod` + labelSuffix + `,namespace,container` + labelSuffix + `)` + querySuffix
	errors += getWorkload(promaddress, "rss_workload", "Actual Memory Utilization", query, "max", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)
	errors += getWorkload(promaddress, "rss_workload", "Prometheus Actual Memory Utilization", query, "avg", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)

	query = queryPrefix + `sum(container_fs_usage_bytes{name!~"k8s_POD_.*"}) by (instance,pod` + labelSuffix + `,namespace,container` + labelSuffix + `)` + querySuffix
	errors += getWorkload(promaddress, "disk_workload", "Raw Disk Utilization", query, "max", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)
	errors += getWorkload(promaddress, "disk_workload", "Prometheus Raw Disk Utilization", query, "avg", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)
	...
	*/
	...
	query = queryPrefix + `sum(irate(kube_pod_container_status_restarts_total{name!~"k8s_POD_.*"}[1h])) by (instance,pod,namespace,container)` + querySuffix
	errors += getWorkload(promaddress, "restarts", "Restarts", query, "max", *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)
	...
	*/
	...
	//HPA workloads
	query = `kube_hpa_spec_max_replicas`
	errors += getHPAWorkload(promaddress, "max_replicas", "Auto Scaling - Maximum Size", query, *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)

	query = `kube_hpa_spec_min_replicas`
	errors += getHPAWorkload(promaddress, "min_replicas", "Auto Scaling - Minimum Size", query, *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)

	query = `kube_hpa_status_condition{status="` + hpaStatus + `",condition="` + hpaCondition + `"}`
	errors += getHPAWorkload(promaddress, "condition_scaling_limited", "Scaling Limited", query, *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)

	query = `kube_hpa_status_current_replicas`
	errors += getHPAWorkload(promaddress, "current_replicas", "Auto Scaling - Total Instances", query, *args.ClusterName, *args.PromAddress, *args.Interval, *args.IntervalSize, *args.History, currentTime)
