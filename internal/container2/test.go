...
type topLevel struct {
	...
	currentSize, creationTime, restarts int
...
func Metrics(clusterName, promProtocol, promAddr, promPort, interval string, intervalSize, history int, debug bool, currentTime time.Time) {
	...
	for i := 0; i < containerRslt.Len(); i++ {
		...
		if _, ok := namespaces[namespaceName]; !ok {
			namespaces[namespaceName] = &namespace{pointers: map[string]*topLevel{}, topLevels: map[string]*topLevel{}}
		}
		...
		if controllerName, ok := podOwners[podName]; ok {
			if deploymentName, ok := replicaSetOwners[controllerName]; ok && podOwnersKind[podName] == "ReplicaSet" {
				...
				if _, ok := namespaces[namespaceName].topLevels["Deployment__"+deploymentName]; !ok {
					namespaces[namespaceName].topLevels["Deployment__"+deploymentName] = &topLevel{name: deploymentName, kind: "Deployment", containers: map[string]*container{}, labelMap: map[string]string{}}
				...
				if _, ok := namespaces[namespaceName].topLevels["CronJob__"+cronJobName]; !ok {
					namespaces[namespaceName].topLevels["CronJob__"+cronJobName] = &topLevel{name: cronJobName, kind: "CronJob", containers: map[string]*container{}, labelMap: map[string]string{}}
				...
				if _, ok := namespaces[namespaceName].topLevels[podOwnersKind[podName]+"__"+controllerName]; !ok {
					namespaces[namespaceName].topLevels[podOwnersKind[podName]+"__"+controllerName] = &topLevel{name: controllerName, kind: podOwnersKind[podName], containers: map[string]*container{}, labelMap: map[string]string{}}
			...
			if _, ok := namespaces[namespaceName].topLevels["Pod__"+podName]; ok {
				...
				namespaces[namespaceName].topLevels["Pod__"+podName] = &topLevel{name: podName, kind: "Pod", containers: map[string]*container{}, labelMap: map[string]string{}}
			}
